...
func TestByAvgScore(t *testing.T) {
	...
	p := predicates.ByAvgScore(8.0, 9.0) // => true

	if !p(student) {
		t.Error("expected true, got false")
	}

	p1 := predicates.ByAvgScore(6.0, 8.0)
...
func TestByRank(t *testing.T) {
	...
	p := predicates.ByRank(student.Good)
...
func TestAnd(t *testing.T) {
	p := predicates.AndStudent(
		predicates.ByName("Nguyen"),
		predicates.ByClass("10A"),
		predicates.ByAvgScore(7.0, 8.5),
		predicates.ByRank(student.Good),
	)
