...
func NewRouter(h TeacherHandler) *http.ServeMux {
	...
	mux.HandleFunc("GET /teachers/paginated", h.GetTeachersPaginated)
	...
	mux.HandleFunc("POST /teachers/bulk-upload", h.BulkAddTeachers)
	mux.HandleFunc("PUT /teachers/{id}", h.UpdateTeacher)
	mux.HandleFunc("DELETE /teachers/{id}", h.DeleteTeacher)
	mux.HandleFunc("GET /teachers/export", h.ExportTeachers)
